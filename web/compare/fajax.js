      ...
      success: res => {
        if (res.statusCode === 200) { // http请求200
          if (res.data.retcode === '0000') { // 后端接口正常返回
            resolve(res.data)
          } else if (res.data.retcode == '399' || res.data.retcode == '9101') { // 未登录的请求
            //  需要登录
    ...
    wx.login({
      success(res) {
        if (res.code) {
          // 发起网络请求
          wx.request({
            url: host + '/auth/families/mini/login',
            method: 'POST',
            header: {
...
async function toExperience() {
  ...
  isExp = true
  let result = await formRequestSync('/auth/families/mini/tasteLogin', {
    mobile: '15671559316'
  })
